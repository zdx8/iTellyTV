...
| ← (LEFT) | 召唤抽屉 | 收抽屉 |
| ↑ (UP) | 切上一个频道 | 在抽屉里**移高亮**（视频不变） |
| ↓ (DOWN) / → (RIGHT) | 切下一个频道 | 在抽屉里**移高亮**（视频不变） |
| Back | 退出 app | 收抽屉（再按退出） |
| 数字 0-9 | 跳到第 N 个频道 | 同左 |
...
- ChannelNavigation（15 个测试，上下键环绕 / 抽屉光标夹取边界）
